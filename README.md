...
> AI 驱动的全链路设计系统工具链 —— 6 个协作 Skills，覆盖**需求分析 → 视觉创意 → 代码生成 → 合规审计**完整设计工作流。适用于 Claude Code、Cursor、Codex 等主流 AI Coding 工具。

## 这是什么？

AG Design Skills 是一套专为 B 端产品设计的 AI 辅助设计系统。它不是一个组件库，而是 **6 个 AI Skills 组成的智能工作流**，每个 Skill 负责设计流程中的一个环节，通过 8 个标准化接口协作，将模糊的产品需求转化为符合设计规范的生产代码。可配合 Claude Code、Cursor、Windsurf、Codex 等任意支持自定义指令的 AI Coding 工具使用。
...
cp -r ag-design-skills/ag-design-* ~/.claude/skills/
```
...
将仓库克隆到项目目录后，在 Cursor Settings → Rules 中添加项目级规则文件 `.cursorrules`：
